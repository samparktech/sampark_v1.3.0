...
router.post('/create', upload.single('articleImage'),ensureAuthenticated, async (req, res) => {
    ...
    const message = `${req.user.username} posted on Sampark! Check it out ! http://18.204.213.158:8080/`;
    sendSms(req.user.phnumber, message);
...
router.post('/like/:postid/:username', ensureAuthenticated, (req, res) => {
    const postId = req.params.postid;
    const userId = req.params.username;
    const likedBoj = {
      likedBy : userId
    };
    Article.findByIdAndUpdate(postId, {$push : {likes : likedBoj}}, (err, done) => {
      if(done){
        res.redirect('/dashboard');
      }else{
        console.log(err);
